...
namespace DESSAU.ControlGestion.Web.Models.EvaluacionModels
{
    public class DetalleEvaluacionPdfGenerator
    {
        ...
        public void generarDetalleEvaluacionPdf(Document doc, DESSAUControlGestionDataContext db)
        {
            ...
            identificacion.AddCell(getCabeceraHorizontal("Nombre"));
            ...
            doc.Add(new Paragraph("\n\n", FontFactory.GetFont("Arial", 6)));
            doc.Add(new Paragraph(String.Format("Se evalúan {0} competencias para el cargo del evaluado.", _evaluacion.EvaluacionPreguntas.Count()), Normal));
            ...
            int i = 0;
            foreach (var evaluacionPregunta in _evaluacion.EvaluacionPreguntas)
            {
            ...
            for (int j = 1; j <= 5; j++)
            {
                evaluacionTable.AddCell(
                    getStripedNormalCell(
                        _evaluacion.EvaluacionPreguntas.Count(x => x.ValorObtenido == j).ToString("N0"),
                        i));
            }
            evaluacionTable.AddCell(
                    getStripedNormalCell(
                        _evaluacion.EvaluacionPreguntas.Count().ToString("N0"), i));
            i++;
